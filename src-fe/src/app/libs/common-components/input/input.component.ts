...
import { InputType } from '../common/enum/input-type.enum';
import { PasswordVisibility } from '../common/enum/password-visibility.enum';
...
export class InputComponent implements ControlValueAccessor, OnInit, OnDestroy, AfterViewInit {
	...
	public onValueChange(value: any): void {
		...
		if (Boolean(this.value) && this.value !== value) {
			console.log('input valueChange', value);
			this.setValue(value);
			this.valueChange.emit(value);
		}
		this.validate();
	...
	public validate(): void {
		...
		this.invokeAsyncValidators().then(() => {
	...
	private setSyncValidator(): void {
		this.syncValidators = [].concat(this.inputValidators, this.changeValidators).filter((validator: ValidatorFn | null) => validator);
	}
	private setAsyncValidator(): void {
		this.asyncValidators = []
			.concat(this.inputValidatorsAsync, this.changeValidatorsAsync)
	...
	private promiseToObservable(
		error: Promise<ValidationErrors | null> | Observable<ValidationErrors | null>
